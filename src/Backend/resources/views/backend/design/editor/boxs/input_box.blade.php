...
<div class="theme-setting theme-setting--text editor-item">
    ...
    <p><a href="javascript:void(0)" class="load-media" data-input="input-{{ $index }}-{{ $icard }}-{{ $iinput }}" data-preview="review-{{ $index }}-{{ $icard }}-{{ $iinput }}"><i class="fa fa-edit"></i> {{ trans('juzacms::app.change') }}</a></p>
    ...
    <select name="{{ $input_name }}" class="load-genres">
        @if($input_value)
            @php
                $genre = \Juzaweb\Cms\PostType\Models\Taxonomy::where('id', '=', $input_value)
                    ->first(['name']);
            @endphp
    ...
    <select name="{{ $input_name }}[]" class="load-genres" multiple>
        @if($input_value)
            @php
                $genres = \Juzaweb\Cms\PostType\Models\Taxonomy::whereIn('id', $input_value)
                    ->get(['id', 'name']);
            @endphp
