    ...
    <div class="theme-editor__panel" id="panel-{{ $index }}" tabindex="-1">
        ...
        <div class="theme-editor__panel-body" data-scrollable>
            <form action="{{ route('admin.design.editor.save') }}" method="post" class="form-ajax" data-success="save_success">
                <button class="btn btn-save-top" type="submit">{{ trans('juzacms::app.save') }}</button>
                        ...
                        <section class="next-card theme-editor__card card-{{ $index }}-{{ $icard }}">
                            <section class="next-card__section">
                                ...
                                <header class="next-card__header theme-setting theme-setting--header">
                                    <h3 class="ui-subheading">{{ $card['name'] }} <a href="javascript:void(0)" class="show-card-body"><i class="fa fa-eye"></i> {{ trans('juzacms::app.show') }}</a></h3>
                                </header>
                                    ...
                                    @if(isset($card['input_items']))
                                        @foreach($card['input_items'] as $iinput => $input)
                                            @if(in_array($input['element'], ['input', 'textarea', 'media', 'slider', 'select_genre', 'select_genres']))
                                                ...
                                                @include('juzacms::backend.design.editor.boxs.input_box')

                                            @else

                                                @include('juzacms::backend.design.editor.boxs.'. $input['element'] .'_box')
                ...
                <button class="btn btn--full-width" type="submit">{{ trans('juzacms::app.save') }}</button>
            </form>
        </div>
    </div>
@endforeach
