<form method="post" action="{{ route('admin.setting.save') }}" class="form-ajax">
    ...
    <h5>@lang('juzacms::app.google_recaptcha')</h5>
    @php
    $google_recaptcha = get_config('google_recaptcha');
    @endphp
    <div class="form-group">
        <label class="col-form-label" for="google_recaptcha">@lang('juzacms::app.google_recaptcha')</label>
        <select name="google_recaptcha" id="google_recaptcha" class="form-control">
            <option value="1" @if($google_recaptcha == 1) selected @endif>@lang('juzacms::app.enabled')</option>
            <option value="0" @if($google_recaptcha == 0) selected @endif>@lang('juzacms::app.disabled')</option>
        </select>
    </div>

    <div class="form-group">
        <label class="col-form-label" for="google_recaptcha_key">@lang('juzacms::app.google_recaptcha_key')</label>
    ...
    <div class="form-group">
        <label class="col-form-label" for="google_recaptcha_secret">@lang('juzacms::app.google_recaptcha_secret')</label>
