...
class BTaxonomyTest extends TestCase
{
    ...
    protected function updateTest($taxonomy)
    {
        if ($tax = $this->makeFactory($taxonomy)) {
            ...
            $response = $this->get($this->getUrlTaxonomy($taxonomy) . '/' . $model->id . '/edit');

            $response->assertStatus(200);

            $this->put($this->getUrlTaxonomy($taxonomy) . '/' . $model->id, $tax->getAttributes());
    ...
    {
        try {
            $post = factory($taxonomy->get('model'))->make();
